...
Grid.mixin({
	...
	eventsToInverseRanges: function(events) {
		...
		for (i = 0; i < normalRanges.length; i+t = viewStart; // the end of the previous range. the start of the new range
		var i, normalRange;
...
function EventManager(options) { // assumed to be a calendar
	...
	function removeEvents(filter) {
		...
		if (filter == null) { // null or undefined. remove all events
			...
			eventID = filter + '';
			fo official Event Objects up front.
	...
	// `even with an end for isSelectionRangeAllowed
