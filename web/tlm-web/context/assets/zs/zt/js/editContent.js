define(["validate.additional", "select3", "ztree.select"], function(){
	...
	var editor = null;
	var isAdd = true;
	var addUrl = "zs/zt/content/add";
	var updateUrl = "zs/zt/content/update";
	var id = -1;
	var rwId = null;
	...
	var showSelectZt = function(params, cb){
		App.ajaxModal({
			id: "selectZt",
			scroll: true,
			width: "600",
			referer: "editContent",
			remote: basePath+"assets/zs/zt/selectZt.html",
			...
			if(_data && _data.pid && _data.pid > 0){
				...
				$form.find('.btn.select').on('click', function(){
					showSelectZt({
						...
						xq: _data.xq,
						qzqm: _data.qzqm
					}, function(ztId, ztName){
						$form.find('input[name="pid"]').val(ztId);
