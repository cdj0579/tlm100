(function( factory ) {
	...
	var submitZd = function(form, m){
		...
		form.ajaxSubmit({
    		...
    		type: "POST",
    		dataType: "json",
    		data: {tableName: self.tableName, nameField: self.nameField, idField: self.idField, typeField: self.typeField, typeVelue: self.typeVelue},
		...
		var zdForm = $(
			'<form class="zd-group form-group">'+
				'<div class="zdadd-wrapper input-group">'+
					...
					'<div class="input-icon right"><i class="fa"></i><input type="text" name="value" class="value form-control" validate="{required: true}" placeholder="请输入要添加的'+this.zdLabelText+'"/></div>'+
	...
	var onClick = function(){
		...
		App.ajaxModal({
			id: "zdadd_modal",
			remote: basePath+"assets/common/ui/zd/zdadd.html",
			callback: function(modal){
				modal.$element.find('span.zd-label-text').html(self.zdLabelText);
	...
	var reload = function(v, options){
		...
		var params = {tableName: this.tableName, idField: this.idField, nameField: this.nameField, typeField: this.typeField, typeVelue: this.typeVelue};
		if(this.group == true){
