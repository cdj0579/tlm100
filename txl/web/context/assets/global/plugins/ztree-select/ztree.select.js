		...
		ZtreeSelect.prototype = {
			...
			initTree: function(){
				...
				this.load = function(params){
					var data = $.extend({}, this.options.baseParams, params);
					$.getJSON(this.options.url, data, function(json){
						if(json.success == true){
							var rows = null;
							if($.isFunction(_this.options.dataParam)){
								rows = _this.options.dataParam(json);
							} else if(json[_this.options.dataParam]){
								rows = json[_this.options.dataParam];
							}
							if(rows){
								cb(rows);
							}
				...
				} else if(this.options.url && this.options.autoLoad == true){
					this.load({});
				}
