...
public class AppIndexService {
	 ...
	 * @throws Exception
	 */
	public Map<String, Object> queryOneFenPeiLianxiren(String syzId,String fpId) throws Exception{
		Connection conn = null;
		PreparedStatement stmt = null;
		ResultSet rs = null;
		...
		sqlSb.append(" select  T0.id as fpId ,T0.lxr_id as lxrId,T1.lianxiren as name,T1.xingbie as sex,T1.dq_id quyu,")
			...
			.append(" from txl_lianxiren_fenpei T0 right join txl_lianxiren T1 on T1.id = T0.lxr_id ")
			.append(" left join txl_xuexiao T2 on T2.id=T1.xuexiao_id where T0.syz_id=? and T0.id >? order by T0.id limit 1");
		try {
			conn = DBFactory.getConn();
			stmt = conn.prepareStatement(sqlSb.toString());
			stmt.setString(1, syzId);
			stmt.setString(2, fpId);
			rs = stmt.executeQuery();
			if(rs != null && rs.next()){
	...
	public List<Map<String, Object>> queryGzOrGxLianxiren(String syzId,boolean isGx) throws Exception{
		Connection conn = null;
		PreparedStatement stmt = null;
		ResultSet rs = null;
		...
		sqlSb.append(" T0 right join txl_lianxiren T1 on T1.id = T0.lxr_id ")
...
//		service.updateLianxirenBeizhu(1, DateUtils.formatDateToHMS(new Date()));
		...
		System.out.println(service.queryOneFenPeiLianxiren("1", "0"));
